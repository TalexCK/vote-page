...
export function PollPage({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        ...
        if (nextPoll && nextPoll.status === 'ended') {
  ...
  const showResults = poll?.status === 'ended'
  ...
  return <main className="page-enter mx-auto w-full max-w-3xl flex-1 px-6 pb-12 pt-12 sm:pt-20">
    ...
    {poll && <>
      <div className="mb-10 sm:mb-14">
        <div className="mb-6 flex items-center justify-between gap-4"><p className="eyebrow">{showResults ? '投票结果' : '投票'}</p><span className="flex items-center gap-2 rounded-full border px-3 py-1 text-xs"><span className={`size-1.5 rounded-full ${poll.status === 'open' ? 'bg-foreground' : 'bg-neutral-400'}`} />{{ pending: '未开始', open: '进行中', ended: '已结束' }[poll.status]}</span></div>
