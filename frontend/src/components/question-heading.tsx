...
export function QuestionHeading({ question, index }: { question: Question; index: number }) {
  return <div className="mb-6 flex gap-4 sm:gap-6">
    ...
    <div className="min-w-0 flex-1">
      <div className="mb-2.5 flex flex-wrap items-center gap-2 text-xs"><span>{question.type === 'single' ? '单选' : '多选'}</span><span className="text-neutral-300">/</span><span className={question.required ? 'font-medium' : 'text-muted-foreground'}>{question.required ? '必填' : '选填'}</span></div>
